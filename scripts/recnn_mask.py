...
from run import train, test, extract_features, extract_masks, save_checkpoint, find_bounds_clr
import scipy.io
...
parser.add_argument('-f', '--extract_features', dest='extract_features', action='store_true', help='Just evaluate model')
...
parser.add_argument('--setting', type=str, default='leave_out.mat', help='cross validation or leave out settings')
...
def main():
    ...
    if args.extract_features:
        ...
        (test_features, test_preds, test_target) = extract_features(model, ex_model, val_loader, criterion, device, dtype)

        test_features_numpy = test_features.cpu().numpy()
        test_preds_numpy = test_preds.cpu().numpy()
        test_target_numpy = test_target.cpu().numpy()

        test_data = {'test_features':test_features_numpy, 'test_preds':test_preds_numpy, 'test_target':test_target_numpy}
        ...
        train_loader = torch.utils.data.DataLoader(trainxy, batch_size=args.batch_size, shuffle=False)

        (train_features, train_preds, train_target) = extract_features(model, ex_model, train_loader, criterion, device, dtype)
        ...
        train_data = {'train_features':train_features_numpy, 'train_preds':train_preds_numpy, 'train_target':train_target_numpy}
